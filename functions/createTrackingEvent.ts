...
export default CreateTrackingEvent(async ({name, metadata, projectId}, api) => {
  const createResult = await api.models.trackingEvent.create({
    name: name,
    projectId: projectId,
    metadata: metadata,
  })
  ...
  const pusherChannel = "proj-" + projectId
  const pusherEvent = "track"
  
  const pushResult = await pusher.trigger(pusherChannel, pusherEvent, createResultObj);
  if (!pushResult.ok) {
    return {
      errors: [{
